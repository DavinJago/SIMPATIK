...
<head>
    ...
    <title>SIMPATIK - Bank Soal</title>
...
<body class="bg-zinc-900 text-zinc-100 min-h-screen flex">
    ...
    <aside class="w-56 min-h-screen border-r flex flex-col py-6 px-4 fixed top-0 left-0 h-full shadow-sm z-20" style="background-color: #D9D9D9;">
        ...
        <nav class="flex flex-col gap-1 flex-1">
            ...
            $menus = [
                ...
                ['icon' => 'Bank.png',       'label' => 'Bank Soal',   'url' => '/banksoal',  'active' => true],
            ];
            @endphp
    ...
    <main class="ml-56 flex-1 p-8">
        ...
        <div class="flex items-center justify-between mb-8">
            <div>
                <h1 class="text-2xl font-bold text-white">Bank Soal</h1>
                <p class="text-sm text-zinc-400 mt-0.5">Kelola soal dan modul praktikum</p>
            </div>
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr style="background-color: #8B8B8B; border-bottom: 1px solid #6b6b6b;">
                        <th class="text-left px-5 py-3.5 text-xs font-semibold uppercase tracking-wider text-white">Judul Modul</th>
                        <th class="text-left px-5 py-3.5 text-xs font-semibold uppercase tracking-wider text-white">Mata Kuliah</th>
                        <th class="text-left px-5 py-3.5 text-xs font-semibold uppercase tracking-wider text-white">Jumlah Soal</th>
                        <th class="text-left px-5 py-3.5 text-xs font-semibold uppercase tracking-wider text-white">Tanggal Diubah</th>
                        <th class="text-left px-5 py-3.5 text-xs font-semibold uppercase tracking-wider text-white">Aksi</th>
                    </tr>
                </thead>
                <tbody class="divide-y" style="border-color: #7a7a7a;">
                    ...
                    <tr class="transition-colors" style="background-color: #D9D9D9;"
                        ...
                        <td class="px-5 py-4 text-zinc-800 font-medium">{{ $soal->judul }}</td>
                        <td class="px-5 py-4">
                            @if($soal->mata_kuliah)
                                <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-semibold text-zinc-800" style="background-color: #c4c4c4;">
                                    {{ $soal->mata_kuliah }}
                                </span>
                            @else
                        ...
                        <td class="px-5 py-4 text-zinc-600 text-xs">
                            {{ $soal->updated_at->translatedFormat('d F Y') }}
                        </td>
                        <td class="px-5 py-4">
                            <div class="flex items-center gap-2">
                                <button @click="selected = {{ $soal }}; showEdit = true" type="button"
                                    class="p-1.5 rounded-lg text-blue-500 hover:bg-blue-50 transition-colors">
                                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                    </svg>
                                </button>
                                <button @click="selected = {{ $soal }}; showHapus = true" type="button"
                    ...
                    @empty
                    <tr>
                        <td colspan="5" class="px-5 py-12 text-center text-zinc-500 text-sm" style="background-color: #D9D9D9;">
                            <svg class="w-10 h-10 mx-auto mb-2 opacity-30 text-zinc-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                            Belum ada bank soal. Tambahkan sekarang!
                        </td>
                    </tr>
                    @endforelse
                </tbody>
        ...
        <p class="text-xs text-zinc-400 mt-4">Menampilkan {{ $soals->count() }} soal</p>
    </main>
    ...
    <div x-show="showTambah" x-cloak class="fixed inset-0 z-50 flex items-center justify-center">
        ...
        <div class="relative rounded-2xl shadow-xl w-full max-w-md mx-4 p-6" style="background-color: #D9D9D9;">
            <h2 class="text-lg font-bold text-zinc-900 mb-5">Tambah Bank Soal</h2>
            <form action="{{ route('banksoal.store') }}" method="POST" class="space-y-4">
                @csrf
                <div>
                    <label class="block text-xs font-semibold text-zinc-500 mb-1.5">Judul Modul</label>
                    <input type="text" name="judul" required placeholder="cth. Modul 3: Sorting Algorithm"
                ...
                <div>
                    <label class="block text-xs font-semibold text-zinc-500 mb-1.5">Jumlah Soal</label>
                    <input type="number" name="jumlah_soal" required min="1" placeholder="cth. 25"
                        class="w-full px-4 py-2.5 rounded-xl border border-zinc-300 text-zinc-800 text-sm focus:outline-none focus:ring-2 focus:ring-green-400" style="background-color:#fff;">
                </div>
                <div>
                    <label class="block text-xs font-semibold text-zinc-500 mb-1.5">Praktikum</label>
                    <select name="praktikum"
                        class="w-full px-4 py-2.5 rounded-xl border border-zinc-300 text-zinc-800 text-sm focus:outline-none focus:ring-2 focus:ring-green-400" style="background-color:#fff;">
                        <option value="">-- Pilih Praktikum --</option>
                        <option value="Praktikum TI">Praktikum TI</option>
                        <option value="Praktikum TI'25">Praktikum TI'25</option>
                        <option value="Praktikum TI'24">Praktikum TI'24</option>
                        <option value="Praktikum TI'23">Praktikum TI'23</option>
                    </select>
                </div>
                <div>
    ...
    <div x-show="showEdit" x-cloak class="fixed inset-0 z-50 flex items-center justify-center">
        ...
        <div class="relative rounded-2xl shadow-xl w-full max-w-md mx-4 p-6" style="background-color: #D9D9D9;">
            <h2 class="text-lg font-bold text-zinc-900 mb-5">Edit Bank Soal</h2>
            <form :action="`/banksoal/${selected?.id}`" method="POST" class="space-y-4">
                @csrf
                @method('PUT')
                <div>
                    <label class="block text-xs font-semibold text-zinc-500 mb-1.5">Judul Modul</label>
                    <input type="text" name="judul" required :value="selected?.judul"
                ...
                <div>
                    <label class="block text-xs font-semibold text-zinc-500 mb-1.5">Jumlah Soal</label>
                    <input type="number" name="jumlah_soal" required min="1" :value="selected?.jumlah_soal"
                        class="w-full px-4 py-2.5 rounded-xl border border-zinc-300 text-zinc-800 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400" style="background-color:#fff;">
                </div>
                <div>
                    <label class="block text-xs font-semibold text-zinc-500 mb-1.5">Praktikum</label>
                    <select name="praktikum"
                        class="w-full px-4 py-2.5 rounded-xl border border-zinc-300 text-zinc-800 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400" style="background-color:#fff;">
                        <option value="">-- Pilih Praktikum --</option>
                        @foreach(["Praktikum TI", "Praktikum TI'25", "Praktikum TI'24", "Praktikum TI'23"] as $p)
                        <option value="{{ $p }}" x-bind:selected="selected?.praktikum === '{{ $p }}'">{{ $p }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    ...
                    <select name="mata_kuliah"
                        class="w-full px-4 py-2.5 rounded-xl border border-zinc-300 text-zinc-800 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400" style="background-color:#fff;">
                        <option value="">-- Pilih Mata Kuliah --</option>
                        @foreach(['Rekayasa Perangkat Lunak', 'Struktur Data', 'Pemrograman Dasar'] as $mk)
                        <option value="{{ $mk }}" x-bind:selected="selected?.mata_kuliah === '{{ $mk }}'">{{ $mk }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="flex gap-3 pt-2">
    ...
    <div x-show="showHapus" x-cloak class="fixed inset-0 z-50 flex items-center justify-center">
        ...
        <div class="relative rounded-2xl shadow-xl w-full max-w-sm mx-4 p-6 text-center" style="background-color: #D9D9D9;">
            ...
            <h2 class="text-lg font-bold text-zinc-900 mb-1">Hapus Bank Soal?</h2>
            <p class="text-sm text-zinc-500 mb-6">Modul <span class="font-semibold text-zinc-700" x-text="selected?.judul"></span> akan dihapus permanen.</p>
            <form :action="`/banksoal/${selected?.id}`" method="POST">
                @csrf
                @method('DELETE')
